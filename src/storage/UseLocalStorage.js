  ...
  if (action.type === "LOGIN") {
    ...
    if (jsonValue === null) {
      localStorage.setItem(
        prefixedKey,
        JSON.stringify([{ username: info.username, money: 99.99 }])
      );
      return { username: info.username, money: 99.99 };
    } else {
      ...
      if (
        ...
        localStorage.setItem(
          ...
          JSON.stringify([
            ...newJsonValue,
            { username: info.username, money: 99.99 },
          ])
        );
        return { username: info.username, money: 99.99 };
      } else {
...
export function gameHistoryStorage() {
  const tableHistory =
    JSON.parse(localStorage.getItem("paktolus-GAME-HISTORY")) || [];
  return tableHistory;
}
